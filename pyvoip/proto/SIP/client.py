...
class SIPClient:
    ...
    def gen_bye(self, request: SIPMessage) -> str:
        ...
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # For outgoing Bye in UAS part of INVITE transaction,
        # (we received INVITE, but send BYE)
        # there is not tag in the To header
        # Probably due to the fact that is is generated by the
        # Incoming INVITE where there is not out tag set.
        # Hence, the FROM tag should be set to tag.
        # from the Taglibrary
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        if request.headers["From"]["tag"] == tag:
            ...
            f_tag = tag
            t_tag = request.headers["From"]["tag"]
            t_user = self.user
            t_name = self.user
            t_domain = self.server
